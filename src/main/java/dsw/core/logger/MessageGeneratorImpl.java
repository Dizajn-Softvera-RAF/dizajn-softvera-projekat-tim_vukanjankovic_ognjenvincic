...
public class MessageGeneratorImpl implements IPublisher, MessageGenerator {
    ...
    public void generateMessage(MessageType type) {
        ...
        if(type == MessageType.CANT_ADD) msg = new Message(Message.ERROR, "CANT ADD THIS ITEM HERE");
        if(type == MessageType.CANT_ADD_PACKAGE_IN_PROJECTEXPLORER) msg = new Message(Message.ERROR, "CANT ADD PACKAGE IN PROJECT EXPLORER");
        if(type == MessageType.CANT_ADD_DIAGRAM_IN_PROJECTEXPLORER) msg = new Message(Message.ERROR, "CANT ADD DIAGRAM IN PROJECT EXPLORER");
